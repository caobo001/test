        ...
        <table class="table table-hover text-center" v-if="sortedValue.length">
          <thead>
            <tr>
              ...
              <th class="text-center">平均阅读</th>
              <th class="text-center">总评论数</th>
              <th class="text-center">平均阅读</th>
          ...
          <tbody>
            <tr v-for="(item, index) in sortedValue" :key="item.fdGoodsId">
              ...
              <td>
                <div class="layer-fix">
                  <img :src="item.fdImage.thumbnailUrl" class="avatar vertical-middle">
                  <div class="inline-block vertical-middle">
                    <router-link class="ellipsis name vertical-middle" :to="{ name: 'media-billboard-headline-detail', params: {id: item.fdGoodsId}}" :title="item.fdName">
                      {{item.fdName}}
                    </router-link>
                    <img
              ...
              <td>{{item.fdReadTotal | numToMyriad}}</td>
              <td>{{item.fdAvgRead | numToMyriad}}</td>
              <td>{{item.fdCommentTotal | numToMyriad}}</td>
              <td>{{item.fdAvgComment | numToMyriad}}</td>
...
export default {
  asyncData ({ store, route }) {
    return Promise.all([
      ...
      store.dispatch('billboard/fetchHeadlineList'),
  ...
  methods: {
    ...
    ...mapActions({
      fetchList: 'billboard/fetchHeadlineList',
    ...
    search () {
      ...
      this.$router.push({name: 'media-billboard-headline-search', params: { keyword: this.keyword }})
    },
    toggleCart (obj) {
    ...
    querySearchAsync (queryString, cb) {
      ...
      '/goods/search', {
        params: {
          fdName: queryString,
          fdGoodsGroupId: this.$store.state.goodsGroups.headline
        }
      }).then((resp) => {
  ...
  watch: {
    ...
    checkedTrades (newValue) {
      ...
      this.$store.dispatch('billboard/fetchHeadlineList').finally(() => {
        this.loading = false
      })
    }
  },
