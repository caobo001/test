  ...
  <div class="filter-box">
    ...
    <div class="section clearfix">
      ...
      <div class="content pull-right other-filter">
        <el-checkbox class="check-box" v-model="isOriginal">原创</el-checkbox>
        <el-checkbox class="check-box" v-model="isCommented">带评论</el-checkbox>
        <div class="range">
          易指数
        ...
        <div class="range">
          发布频率
          <el-input-number v-model="publishRange.gt" :min="0"></el-input-number>至
          <el-input-number v-model="publishRange.lt" :min="0"></el-input-number>
          <div class="range-in">
            <span class="clear-btn" @click="clearRange(publishRange)">清空</span>
            <button class="confirm-btn" @click="setRange(publishRange)">确定</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="selected-opts" v-show="showSelected">
    ...
    <ul class="inline-block">
      ...
      <li v-show="filterKeys.readRange.text !== ''">
        最近阅读量：{{filterKeys.readRange.text}}
        <i class="iconfont icon-delete" @click="clearFilter('read')"></i>
      </li>
      <li v-show="filterKeys.publishRange.text !== ''">
        发布频率：{{filterKeys.publishRange.text}}
        <i class="iconfont icon-delete" @click="clearFilter('publish')"></i>
      </li>
      ...
      <li v-show="filterKeys.isOriginal">
        原创
        <i class="iconfont icon-delete" @click="clearFilter('isOriginal')"></i>
      </li>
      <li v-show="filterKeys.isCommented">
        带评论
        <i class="iconfont icon-delete" @click="clearFilter('isCommented')"></i>
      </li>
...
export default {
  name: 'CFilter',
  mounted () {
    this.$store.dispatch('fetchTrades').then(() => {
      this.optTrades = Utils.deepClone(this.$store.state.trades)
      this.optTrades.push({text: '地方大号'})
    })
    ...
    if (this.$route.query.local) {
      if (this.trades.indexOf('地方大号') < 0) {
        this.trades.push('地方大号')
      }
    }
  ...
  computed: {
    filterKeys () {
      var writerType, priceRange, area, goodSpecId, fanRange, advType, isOriginal
      var isCommented, gtRead, ltRead, gtPublish, ltPublish, readRange, publishRange
      ...
      gtPublish = this.publishRange.ngt
      ltPublish = this.publishRange.nlt
    ...
    showSelected () {
      return !!this.filterKeys.trades.length ||
        ...
        this.filterKeys.publishRange.text !== '' ||
        this.filterKeys.weightRange.text !== '' ||
        !!this.filterKeys.isOriginal ||
        !!this.filterKeys.isCommented
    },
    optProvince () {
  ...
  watch: {
    ...
    filterKeys (newValue) {
      ...
      Utils.deepClone({
        ...
        isOriginal: newValue.isOriginal,
        isCommented: newValue.isCommented,
    ...
    clearFilter (label) {
      switch (label) {
        ...
        case 'read':
          ...
          break
        case 'publish':
          this.clearRange(this.publishRange)
          break
        case 'weight':
