  ...
  <div class='container'>
    <div class="invoice-info clearfix">
      <div class="section left">
        ...
        <ul class="table">
          <li class="row">
            <span class="label-name">发票金额</span>
            <div class="label-con color-primary">
              ￥{{dataInfo.invoice.fdPrice | numToThousands}}
            </div>
          </li>
          <li class="row">
            <span class="label-name">申请状态</span>
            <div class="label-con color-primary">
              {{dataInfo.invoice.fdStatusValue}}
            </div>
          </li>
          <li class="row">
            <span class="label-name">发票编号</span>
            <div class="label-con">
              {{dataInfo.invoice.fdNo}}
            </div>
          </li>
          <li class="row">
            <span class="label-name">申请时间</span>
            <div class="label-con">
              {{dataInfo.invoice.createdDate}}
            </div>
          </li>
        </ul>
      </div>
      <div class="section right">
        ...
        <ul class="table">
          <li class="row">
            <span class="label-name">物流公司</span>
            <div class="label-con">
              {{dataInfo.carriage.fdLogistics || '—'}}
            </div>
          </li>
          <li class="row">
            <span class="label-name">运单号</span>
            <div class="label-con">
              {{dataInfo.carriage.fdLogisticsNo || '—'}}
            </div>
          </li>
        </ul>
      </div>
    ...
    <div class="invoice-info clearfix">
      <div class="section left top">
        ...
        <ul class="table">
          ...
          <li class="row" style="display:flex" v-if="invoiceInfo.fdBillingInfo && invoiceInfo.fdBillingInfo.length>0">
            <span class="label-name">开票信息</span>
            <ul class="label-con attachment" style="margin-left:4px">
             <!--  <li v-for="(item, index) in invoiceInfo.fdBillingInfo" :key="index">
                <div class="preview preview-diff" v-for="(item, index) in invoiceInfo.fdBillingInfo" :key="index">
                  <c-image-zoom
                  ref="fdBillingInfo"
              ...
              <c-upload-preview v-model="invoiceInfo.fdBillingInfo"
               model="file" :showDelBtn="false" :text-max-width="true" style="display: grid"></c-upload-preview>
            </ul>
          </li>
        </ul>
      </div>
          ...
          <tr v-for="item in orderList" :key="item.id">
            ...
            <td class="brief">
              <div
                ...
                class="media"
                v-if="item.goodsInfo.fdGoodsGroupId === wechat">
              ...
              <div
                class="media"
                v-if="item.goodsInfo.fdGoodsGroupId === weibo"
...
export default {
  ...
  computed: {
    invoiceInfo () {
      if (_.isEmpty(this.dataInfo.invoiceInfo)) {
        return {
          fdAttrKhxk: [],
          fdAttrYbnsr: [],
          fdAttrYyzz: [],
        ...
        return this.dataInfo.invoiceInfo
      }
    },
    orderList () {
      return this.dataInfo.orderList
    }
  },
  components: {
    CImageZoom,
