        ...
        <table class="table table-hover text-center" v-if="sortedValue.length">
          ...
          <tbody>
            <tr v-for="(item, index) in sortedValue" :key="item.fdGoodsId">
              ...
              <td>
                <div class="layer-fix clearfix">
                    <img :src="item.fdImage.thumbnailUrl" class="avatar  vertical-middle inline-block">

                    <router-link class="ellipsis vertical-middle weibo-name" :to="{ name: 'media-billboard-weibo-detail', params: {id: item.fdGoodsId}}" :title="item.fdName">
                      {{item.fdName}}
                    </router-link>
                    <img
              ...
              <td>{{item.fdArticleTotal}}</td>
              <td>{{item.fdFunsCount | numToMyriad}}</td>
              <td>{{item. fdLikeTotal | numToMyriad}}</td>
              <td>{{item.fdCommentTotal | numToMyriad}}</td>
              <td>{{item.fdForwardTotal | numToMyriad}}</td>
...
export default {
  asyncData ({ store, route }) {
    return Promise.all([
      ...
      store.dispatch('billboard/fetchWeiboList'),
  ...
  computed: {
    ...mapState('billboard', {
      weiboList: state => state.weiboList,
    ...
    sortedValue () {
      var value = utils.deepClone(this.weiboList)
  ...
  methods: {
    ...
    ...mapActions({
      fetchWeiboList: 'billboard/fetchWeiboList',
    ...
    search () {
      ...
      this.$router.push({name: 'media-billboard-weibo-search', params: { keyword: this.keyword }})
    },
    toggleCart (obj) {
      if (obj.isInCart) {
  ...
  watch: {
    ...
    checkedTrades (newValue) {
      ...
      this.$store.dispatch('billboard/fetchWeiboList').finally(() => {
        this.loading = false
      })
    }
  },
